<?php
class Participant_du_labo extends Model {
	private $id;
	private $numero;
	private $appel;
	private $organisme;
	...
	public function addToDB() {
	    ...
	    return $res==1?"Participant du labo créé !":"Bizarre, on ne sait pas ce qu'il s'est passé...";
    } else {
    	throw new Exception("Il manque des informations pour enregistrer ce nouveau participant du labo en base de données");
    }
	}
    ...
    $query = $this->db->prepare($sql);
    $res = $query->execute($params);
    return $res==True && $query->rowCount()==1?"Participant du labo supprimé avec succès":"ERREUR: Ce Participant du labo a sûrement déjà été supprimé";
